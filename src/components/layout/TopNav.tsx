...
import { Search, Bell, ShieldCheck, User, ChevronDown, Sparkles, Cpu } from 'lucide-react';
import MLInspectorModal from '@/components/ml/MLInspectorModal';
  ...
  return (
    <>
      <header className="h-16 border-b border-slate-800/80 bg-slate-950/80 backdrop-blur-md px-6 flex items-center justify-between sticky top-0 z-30">
        ...
        {/* Right: ML Inspector Badge, Security Badge & User Avatar */}
        <div className="flex items-center gap-3">
          {/* ML Model Inspector Trigger Button */}
          <button
            onClick={() => setShowMLModal(true)}
            className="flex items-center gap-1.5 text-xs text-indigo-400 bg-indigo-500/10 hover:bg-indigo-500/20 px-3 py-1.5 rounded-xl border border-indigo-500/30 font-mono transition-all hover:scale-[1.02] shadow-sm shadow-indigo-500/10"
            title="Inspect Machine Learning Model Architecture & Confusion Matrix"
          >
            <Cpu className="w-3.5 h-3.5 text-indigo-400 animate-pulse" />
            <span className="hidden sm:inline">ML Inspector</span>
            <span className="text-[10px] font-bold bg-indigo-500/20 text-indigo-300 px-1.5 py-0.2 rounded">
              93.8%
            </span>
          </button>

          <div className="hidden lg:flex items-center gap-1.5 text-xs text-emerald-400 bg-emerald-500/10 px-2.5 py-1.5 rounded-xl border border-emerald-500/20 font-mono">
            <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
            <span>Zero-Knowledge WS</span>
          </div>
      ...
      {/* Interactive Machine Learning Inspector Modal */}
      <MLInspectorModal isOpen={showMLModal} onClose={() => setShowMLModal(false)} />
    </>
  );
}
